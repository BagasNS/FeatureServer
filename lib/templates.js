...
function _generateServerInfo(params = {}){
  const json = _.cloneDeep(templates.server);
  json.copyrightText = params.copyrightText || json.copyrightText;
  json.description = params.description || json.description;
  return json;
}

function renderServer (data, { layers, tables } = {}, { query = {} } = {}) {
  const json = _generateServerInfo(data);
