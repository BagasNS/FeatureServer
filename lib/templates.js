...
function renderLayer (data = {}, { params = {}, query = {} } = {}) {
  ...
  json.copyrightText = metadata.copyright || process.env.SERVER_COPYRIGHT;
  ...
  if (metadata.description) json.description = metadata.description + process.env.SERVER_COPYRIGHT ? ` | ${process.env.SERVER_COPYRIGHT}` : "";
  if (metadata.idField) {
...
function renderServer (data, { layers, tables } = {}, { query = {} } = {}) {
  ...
  json.copyrightText = process.env.SERVER_COPYRIGHT || json.copyrightText;
  json.description = process.env.SERVER_DESCRIPTION || json.description;
